...
class TestParser(BaseTestAsync):
    ...
    def _assert_parsed_geom(self, obj_feat, feat, fields):
        ...
        for o, ft in zip(obj_feat, feat):
            ...
            if c1.shape != c2.shape:
                ...
                self.assertLess(
                    np.max(np.abs(c1 - c2)), 1e-13, "parsed geometry error > 1e-13"
                )
    ...
    def subtest_parse_xyzjson_map_chunk(self, obj, chunk_size=100):
        similarity_threshold = self.similarity_threshold
        with self.subTest(
            chunk_size=chunk_size, similarity_threshold=similarity_threshold
        ):
            o = dict(obj)
            obj_feat = obj["features"]
            lst_map_feat = list()
            ...
            for i0 in range(0, len(obj_feat), chunk_size):
                ...
                map_feat, _ = parser.xyz_json_to_feature_map(
                    o, map_fields, similarity_threshold
                )
    ...
    def test_parse_qgsfeature(self):
        self.subtest_parse_qgsfeature(
            "geojson-small", "airport-qgis.geojson"
        )  # no xyz_id
