...
class TestTileUtils(BaseTestAsync):
    ...
    def test_bbox_quadkey(self):
        ...
        self.assertEqual(
            sorted(tile_utils.bboxToListQuadkey(*rect_all, 1)), ["0", "1", "2", "3"]
        )
    ...
    def test_bbox_row_col(self):
        ...
        self.assertEqual(
            sorted(tile_utils.bboxToListColRow(*rect_all, 1)), ["1_0_0", "1_1_0"]
        )
        ...
        self.assertEqual(
            sorted(tile_utils.bboxToListColRow(*rect_lowerL, 2)), ["2_0_0", "2_1_0"]
        )
        self.assertEqual(
            sorted(tile_utils.bboxToListColRow(*rect_lowerR, 2)), ["2_2_0", "2_3_0"]
        )
        self.assertEqual(
            sorted(tile_utils.bboxToListColRow(*rect_upperL, 2)), ["2_0_1", "2_1_1"]
        )
        self.assertEqual(
    ...
    def test_coord_from_percent(self):
        ...
        lst_debug_avg = [
            ...
            "expected: \t"
            + "\t".join(map("{:.2f}".format, [0, max_x / 2, 0, max_y / 2])),
        ]
        # print("\n".join(lst_debug_avg))
        try:
            ...
            e.args = (
                linesep.join([e.args[0], *lst_debug_avg, debug_coord]),
                *e.args[1:],
            )
            raise e
