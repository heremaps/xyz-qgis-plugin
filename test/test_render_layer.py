...
class TestRenderLayer(BaseTestAsync):
    ...
    def subtest_render_mixed_json_to_layer_multi_chunk(
        self, obj, ref, lst_chunk_size=None
    ):
    ...
    def subtest_render_mixed_json_to_layer_shuffle(
        self, obj, ref, n_shuffle=5, chunk_size=10
    ):
    ...
    def subtest_render_mixed_json_to_layer_chunk(
        self, obj, chunk_size=100, empty_chunk=None
    ):
        with self.subTest(chunk_size=chunk_size):
    ...
    def _render_layer(self, layer, map_feat, map_fields):
        for geom_str in map_feat:
            for idx, (feat, fields) in enumerate(
                zip(map_feat[geom_str], map_fields[geom_str])
            ):
                vlayer = (
    ...
    def assert_layer(self, layer, obj, map_fields):
        ...
        name_vlayer_fields = dict(
            (geom_str, [x.fields().names() for x in lst])
            for geom_str, lst in map_vlayer.items()
        )
