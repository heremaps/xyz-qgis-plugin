...
class ConnectUX(SpaceUX):
    ...
    def config(self, *a):
        ...
        self.lineEdit_selection.setToolTip(
            "Load only the selected properties of features"
        )
    ...
    def start_connect(self):
        ...
        self.conn_info.set_(
            **meta, token=self.get_input_token(), server=self.get_input_server()
        )
        conn_info = SpaceConnectionInfo(self.conn_info)
        self.signal_space_connect.emit(
            make_qt_args(conn_info, meta, **self.get_params())
        )
