...
USER_AGENT = "xyz-qgis-plugin/{plugin_version} QGIS/{qgis_version} Python/{py_version} Qt/{qt_version}".format(
...
def make_query_url(url, **kw):
    ...
    query = "&".join(
        raw_queries + ["%s=%s" % (k, v) for k, v in kw.items() if not v is None]
    )
    if len(query):
        url = "%s?%s" % (url, query)
    url = QUrl(url)
