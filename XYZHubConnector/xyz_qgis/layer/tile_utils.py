...
def coord_to_percent_bing_reversed(coord, level):
    ...
    y_percent = max(
        0,
        min(1, (0.5 - math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * math.pi))),
    )
...
def coord_to_percent_here_simple(coord, level):
    longitude, latitude = coord
    x_percent = max(0, min(1, ((longitude + 180) / 360)))
    y_percent = max(0, min(1, ((latitude + 90) / 180)))
    return [y_percent, x_percent]
...
def coord_from_percent_here_simple(y_percent, x_percent, level):
    longitude = max(-180, min(180, x_percent * 360 - 180))
    latitude = max(-90, min(90, y_percent * 180 - 90))
...
def parse_tile_id(tile_id, schema="here"):
    return dict(zip(["level", "col", "row"], map(int, tile_id.split("_"))))
...
def bboxToListColRow(x_min, y_min, x_max, y_max, level, schema="here"):
    ...
    return [
        get_tile_format(schema=schema).format(level=level, row=row, col=col)
