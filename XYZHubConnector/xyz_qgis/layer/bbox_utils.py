...
def rect_to_bbox(x_min, y_min, x_max, y_max):
    bbox = dict(zip(["west", "east", "south", "north"], (x_min, x_max, y_min, y_max)))
    # print(bbox)
    return bbox
...
def split_bbox(bbox, nx, ny):
    ...
    def _spiral_pairs():
        it = (
            (x_pairs[ix], y_pairs[iy])
            for ix, iy in spiral_index(len(x_pairs), len(y_pairs))
        )
        return [rect_to_bbox(x[0], y[0], x[1], y[1]) for x, y in it]
