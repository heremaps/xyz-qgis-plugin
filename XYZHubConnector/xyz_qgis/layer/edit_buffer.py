...
from .layer_utils import (
    ...
    make_xyz_id_map_from_src,
    update_feat_non_null,
...
class RollbackTracker(object):
    ...
    def exec_delayed_fun(self):
        ...
        self.is_rollback = not self.is_new_cmd(idx) and self.is_relative_rollback(idx)
        self.idx = idx
        old_a = list()
        new_a = list()
    ...
    def is_new_cmd(self, idx):
        is_new = idx == self.undo_stack.count() and self.cmd != self.undo_stack.command(
            idx - 1
        )
...
class SyncProgress(object):
    ...
    def is_valid_response(self):
        ok = len(self.lst_added_feat) == len(self.lst_added_obj) and len(
            self.lst_removed_obj
        ) == min(1, len(self.lst_removed_ids))
        list(
            map(
                print_qgis,
                [
    ...
    def iter_fid_ft(self):
        ...
        for o1, o2 in zip(self.lst_added_feat, self.lst_added_obj):
            if not (o2 is not None and "features" in o2):
                continue
...
class LayeredEditBuffer(object):
    ...
    def get_xyz_id_(self, lst_fid):
        ...
        for fn in [
            ...
            lambda lst: make_xyz_id_map_from_src(
                get_layer(self.layer_id).dataProvider(), lst
            ),
        ]:
            if len(lst) == 0:
                break
    ...
    def update_xyz_id_added_from_cache(self):
        ...
        added_xyz_ids = dict(
            (k, self.xyz_id_cache[k]) for k in added_ids if k in self.xyz_id_cache
        )
    ...
    def config_connection(self, callback_pairs):
        ...
        for signal, callback in self.callback_pairs:
            signal.connect(
                callback
            )  # Qt.QueuedConnection # error connect failed between geometryChanged and unislot
...
class EditBuffer(object):
    ...
    def config_connection(self, lst_vlayer):
        ...
        for vlayer in lst_vlayer:
            if not (
                isinstance(vlayer, QgsVectorLayer) and is_xyz_supported_layer(vlayer)
            ):
                continue
            layer_id = vlayer.id()
            if layer_id in self.layer_buffer:
                continue

            layer_cache = LayeredEditBuffer(layer_id, cb_enable_ui=self._cb_enable_ui)
    ...
    def make_connection_pair(self, vlayer, layer_cache):
        ...
        return [
            ...
            (
                vlayer.undoStack().indexChanged,
                cb_idx_changed,
            ),  # command shall be executed stack index changed
