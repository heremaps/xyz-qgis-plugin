...
def make_lst_removed_ids(removed_ids):
    ...
    return [
        removed_ids[i : i + chunk_size] for i in range(0, len(removed_ids), chunk_size)
    ]
...
def feature_to_xyz_json(feature, is_new=False, ignore_null=True):
    ...
    exist_feat_id = set()
    return [_single_feature(ft) for ft in feature]
...
def rename_special_props(props):
    ...
    for old in props:
        if old.lower() not in QGS_SPECIAL_KEYS:
            continue
        upper_idx = "".join(str(i) for i, s in enumerate(old) if s.isupper())
        new_name = new_name_fmt.format(old=old, upper_idx=upper_idx)
...
def prepare_fields(feat_json, lst_fields, threshold=0.8):
    ...
    lst_score = [
        fields_similarity((fields.names()), orig_props_names, props_names)
...
def xyz_json_to_feature_map(obj, map_fields=None, similarity_threshold=None):
    ...
    def _single_feature_map(feat_json, map_feat, map_fields):
        ...
        if similarity_threshold is None:
            ...
            fields, idx = prepare_fields(
                feat_json, lst_fields, similarity_threshold / 100
            )
    ...
    if map_fields is None:
        map_fields = dict()
    # map_feat = dict()
    map_feat = dict((k, [list() for i in enumerate(v)]) for k, v in map_fields.items())
