...
class XYZLayer(object):
    ...
    def _save_params_to_node(self, qnode):
        qnode.setCustomProperty(
            QProps.LOADER_PARAMS,
            json.dumps(self.get_loader_params(), ensure_ascii=False),
        )
    ...
    def _save_meta_node(self, qnode):
        qnode.setCustomProperty(
            QProps.LAYER_META, json.dumps(self.meta, ensure_ascii=False)
        )
        qnode.setCustomProperty(
            QProps.CONN_INFO, json.dumps(self.conn_info.to_dict(), ensure_ascii=False)
        )
    ...
    def _make_unique_group_name(self):
        ...
        all_names = sorted(
            self._get_base_group_name(g.name()) for g in tree_root.findGroups()
        )
