...
class EditableItemModel(QStandardItemModel):
    ...
    def get_data(self, row):
        return dict(
            [k, self.get_text(row, col)] for col, k in enumerate(self.INFO_KEYS)
        )
...
class GroupEditableItemModel(EditableItemModel):
    ...
    def from_dict(self, data: GroupedData):
        self.cfg = {
            k: list(filter(self._validate_data, lst)) for k, lst in data.items()
        }
    ...
    def _init_default_servers(self, server_infos: list):
        ...
        for i, server_info in enumerate(server_infos):
            ...
            if not self._validate_data(server_info):
                continue
            it.insertRow(
                i, [QStandardItem(t) for t in self.items_from_data(server_info)]
            )
        self.submit_cache()
...
class ComboBoxProxyModel(QIdentityProxyModel):
    def __init__(
        ...
        token_key="token",
        named_token="{name}",
        nonamed_token="<noname token> {token}",
    ):
        super().__init__()
