...
class LoadLayerController(BaseLoader):
    ...
    def _handle_error(self, err):
        ...
        if isinstance(chain_err, ChainInterrupt):
            ...
            e = chain_err
        if isinstance(
            e, net_handler.NetworkError
        ):  # retry only when network error, not timeout
            reply = e.args[-1]
            self._retry(reply)
            return
...
class UploadLayerController(BaseLoop):
    ...
    def start(
        self, conn_info: SpaceConnectionInfo, lst_added_feat: queue.ParamsQueue, **kw
    ):
