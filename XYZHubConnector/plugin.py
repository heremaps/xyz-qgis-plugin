...
class XYZHubConnector(object):
    ...
    def init_modules(self):
        ...
        ######## token
        self.token_config = ServerTokenConfig(
            config.USER_PLUGIN_DIR + "/token.ini", parent
        )
    ...
    def cb_layer_selected(self, qlayer):
        flag_xyz = (
            True if qlayer is not None and is_xyz_supported_layer(qlayer) else False
        )
        if flag_xyz:
    ...
    def enable_sync_btn(self, flag, msg=""):
        msg = msg or (
            "No changes detected since last push" if not flag else "Push changes"
        )
    ...
    def show_err_msgbar(self, err):
        self.iface.messageBar().pushMessage(
            config.TAG_PLUGIN, repr(err), Qgis.Warning, 3
        )
    ...
    def start_upload_space(self, args):
        ...
        con_upload.signal.results.connect(
            self.make_cb_success_args("Uploading finish", dt=4)
        )
    ...
    def init_all_layer_loader(self):
        ...
        # print_qgis(self.con_man._layer_ptr)
        self.show_success_msgbar(
            "Import XYZ Layer", "%s XYZ Layer imported" % cnt, dt=2
        )
