...
# from .modules.layer_loader import LoadLayerController
...
class XYZHubConnector(object):
    ...
    def initGui(self):
        ...
        # help
            ...
            # self.action_help = QAction(help_icon, "Help", parent)
            # self.action_help.setWhatsThis(QCoreApplication.translate("XYZ Storage",
            #                                                          "XYZ Storage plugin help"))
            # self.action_help.setStatusTip(QCoreApplication.translate("XYZ Storage",
            #                                                          "Get Help on XYZ Storage"))
    ...
    def init_modules(self):
        ...
        # self.render = Render(parent, self.iface)
        # self.net_handler = NetHandler(parent, self.iface)
        # self.parser = XYZParser(parent, self.iface)
    ...
    def unload_modules(self):
        ...
        # when ERROR, restart qgis
        # print("unload_module even if error. When error, con_man del is not available")
        # self.con_man.unload() # consistent
        # del self.con_man # not working when error, (connected signal still get emitted)
        # attributes does not exists
        # for v in ["con_man","network","layer_man"]:
        #     if hasattr(self,v):
        #         delattr(self, v)
    ...
    def unload(self):
        ...
        # self.iface.removeToolBarIcon(self.xyz_widget_action) # remove action from iface (plugin toolbar)
        for a in self.actions:
            self.iface.removePluginWebMenu(self.web_menu, a)
            # self.iface.removeToolBarIcon(a) # remove action from iface (plugin toolbar)
    ...
    def load_bbox(self, con, args):
        ...
        con.start(*a, **kw)
    # deprecated
    # def loader_reload_bbox(self):
    #     bbox = bbox_utils.extend_to_bbox(bbox_utils.get_bounding_box(self.iface))
    #     self.con_man.reload_canvas(bbox=bbox)
    ...
    def open_connection_dialog(self):
        ...
        # ############ bbox btn        
        # con_bbox_reload = ReloadLayerController_bbox(self.network)
        # self.con_man.add_reload(con_bbox_reload)
        # con_bbox_reload.signal.finished.connect( self.refresh_canvas, Qt.QueuedConnection)
        # con_bbox_reload.signal.finished.connect( self.make_cb_success("Reloading finish") )

        # con_bbox_init = InitExtLayerController(self.network)
        # self.con_man.add(con_bbox_init)
        # # con_bbox_init.signal.finished.connect( self.previous_canvas_extent, Qt.QueuedConnection)
        # dialog.signal_space_bbox.connect( lambda args: self.load_bbox(con_bbox_init, args))

        # con_bbox_init.signal.results.connect( self.layer_man.add_args)
        # con_bbox_init.signal.results.connect( con_bbox_reload.start_args)
